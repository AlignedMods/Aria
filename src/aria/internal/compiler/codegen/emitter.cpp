...
#define STR_IDX(idx) (static_cast<OpCode>(m_op_codes.string_table.size() + idx))
#define CONST_IDX(idx) (static_cast<OpCode>(m_op_codes.constant_table.size() + idx))
#define PUSH_OP(op) (m_op_codes.program.push_back(op))
#define PUSH_PENDING_OP(op) (m_pending_op_codes.push_back(op))
...
namespace Aria::Internal {
    ...
    void Emitter::emit_decl_ref_expr(Expr* expr, ExprValueKind value_kind) {
        ...
        if (declRef.referenced_decl->kind == DeclKind::Param) {
            if (value_kind == ExprValueKind::LValue) {
                ...
                PUSH_PENDING_OP(static_cast<OpCode>(m_active_stack_frame.parameters.at(ident)));
            } else if (value_kind == ExprValueKind::RValue) {
                PUSH_PENDING_OP(OP_LD_LOCAL);
                PUSH_PENDING_OP(static_cast<OpCode>(m_active_stack_frame.parameters.at(ident)));
    ...
    void Emitter::emit_temporary_expr(Expr* expr, ExprValueKind value_kind) {
        ...
        OpCode idx = static_cast<OpCode>(m_active_stack_frame.local_count);
    ...
    void Emitter::emit_var_decl(Decl* decl) {
        ...
        if (varDecl.global_var) {
            ...
            PUSH_OP(OP_DECL_LOCAL);
            PUSH_OP(static_cast<OpCode>(m_active_stack_frame.local_count));
            d.data = m_active_stack_frame.local_count;
            m_active_stack_frame.local_count++;
        ...
        if (varDecl.initializer) {
            ...
            if (varDecl.global_var) {
                ...
                PUSH_PENDING_OP(OP_ST_LOCAL);
                PUSH_PENDING_OP(static_cast<OpCode>(m_active_stack_frame.local_count - 1));
            }
        }
    }
    ...
    OpCode Emitter::type_info_to_vm_type_idx(TypeInfo* t) {
        if (t->is_primitive()) {
            return static_cast<OpCode>(m_basic_types[t->kind]);
        }

        if (t->is_structure()) {
            return static_cast<OpCode>(m_structs.at(t->struct_.source_decl).index);
        }
